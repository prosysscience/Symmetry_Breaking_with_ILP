...
class GenerateExample:
	def __init__(self, directory, sat_mode, fullSBC, instance, number_of_cells, maxsizecell, seed, abk, opt):
		if sat_mode:
			self.idAS = 0
		else:
			self.idAS = 1
		...
		self.maxsizecell = maxsizecell
		self.seed = seed
		self.abk = abk
		self.opt = opt
		self.number_of_neg_examples_in_cell = 0
	...
	def compute_examples(self, instance):
		...
		ctl = clingo.Control()
		files = [ self.directory + ASP_FILE_NAME, self.directory + S + instance]
		if self.abk:
			files.append(self.directory + ACTIVE_BK)
		
		# if the program contains optimization statements, compute in "bound" the optimal 
		# values, otherwise it is an empty list
		for file in files:
			ctl.load(file)
		ctl.ground([("base", [])])
		ctl.configuration.solver.rand_freq = "0.5"
		ctl.configuration.asp.eq  = "0"
		...
		with ctl.solve(on_model=lambda m: opt_value.append(m.cost),async_=True) as handle:
			wait = handle.wait(10)
		bound = opt_value.popleft()
		if (not self.opt) or (bound == []):
			ctl.configuration.solve.opt_mode = "enum,{}".format(MAX_OPT_VALUE)
		else:
			ctl.configuration.solve.opt_mode = "enum,{}".format(",".join([str(i) for i in bound]))
		ctl.configuration.solve.models = str(self.number_of_cells)
	...
	def find_symmetric_as(self, start, visited): 
		...
		while queue: 
			...
			if interpretation < self.min_as:
				...
				if self.number_of_neg_examples_in_cell < self.maxsizecell:
					self.write_example("neg", self.min_as, self.atoms_in_all_generators.difference(self.min_as))
				self.number_of_neg_examples_in_cell += 1 
...
# MAIN
"""
def main(directory, sat_mode, fullSBC, instance, number_of_cells, maxsizecell, seed, abk, opt):
	my_generator = GenerateExample(directory, sat_mode, fullSBC, instance, number_of_cells, maxsizecell, seed, abk, opt)
